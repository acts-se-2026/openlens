...
/**
 * A movable, resizable rectangle over a full-bleed photo. Controlled: the caller owns [rect]
 * (normalized 0..1 of THIS overlay's size) and gets every change via [onRectChange]. Drag the body to
 * move; drag a corner/edge handle to resize; the area outside is dimmed. It's normalized to the
 * overlay (not the image), so the frame always stays on screen — the caller maps it to image
 * coordinates for the backend.
 */
@Composable
